...
function md5(input: string): string {
  ...
  function rh(n: number) {
    let j = '', i = -1
    while (++i < 4) j += hc.charAt((n >> (i * 8 + 4)) & 0xf) + hc.charAt((n >> (i * 8)) & 0xf)
    return j
  }
...
const HEADERS: Record<string, string> = {
  ...
  'x-picnic-agent': '30100;1.15.233-15017;',
  'x-picnic-did': 'ILOCARE',
}
...
export function usePicnic(email: string, password: string) {
  ...
  async function searchProduct(token: string, query: string): Promise<string | null> {
    try {
      const res = await fetch(`${BASE}/search?search_term=${encodeURIComponent(query)}`, {
        headers: { ...HEADERS, 'x-picnic-auth': token },
      })
      if (!res.ok) return null
  ...
  async function orderItems(items: string[]) {
    ...
    if (!token) {
      setStatus('❌ Anmeldung fehlgeschlagen – E-Mail/Passwort prüfen')
      setLoading(false)
